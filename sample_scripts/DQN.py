...
class DQN(nn.Module):
    ...
    def __init__(self, n_observations, n_actions):
        ...
        self.layer1 = nn.Linear(n_observations, n_squre)
...
def disp_dots():
    fig = plt.figure(2)
    ui.animate_dots_no_motion([env.dots_kind_matrix, env.dots_kind_matrix_with_candidate], fig=fig, mode='subplot')
    plt.pause(0.001)  # pause a bit so that plots are updated
...
# for i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        if if_disp_dots:
            disp_dots()
            print("reward = " + str(reward) + ", sum_reward = " + str(sum_reward))
